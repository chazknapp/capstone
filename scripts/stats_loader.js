...
(function () {
  ...
  // Prefer the explicit grid if present (for centered 3x2 layout)
  const grid = document.getElementById('statsGrid') || pane;

  // ------------------- Copy blocks (unchanged content) -------------------
  const DESCRIPTIONS = {
    counts_region: [
      '<div class="stat-desc">',
      ...
      ' east.',
      '</p>',
      '<p class="takeaway"><em>The dashed lines show the long-term regression trend for each region.</em></p>',
      '<ul class="bullets">',
    ...
    ef_decade: [
      ...
      '</p>',
      '<p class="takeaway"><em>The dashed regression lines summarize the decline per decade, labeled as <strong>“pp/dec”</strong>, meaning <em>percentage points per decade</em>.</em></p>',
      '<ul class="bullets">',
    ...
    casualties: [
      ...
      ' this speaks to human impact and exposure, it does not by itself measure tornado severity.',
      '</div>'
    ].join(''),
    ...
    regression_summary: [
      ...
      ' the Plains see more off-season storms, the Midwest’s tornadoes are weakening but still frequent, and the Southeast is emerging as the most consistently active and hazardous region.',
      '</div>'
    ].join(''),
  };
  // ----------------------------------------------------------------------

  // Collect cards from the new centered 3x2 grid
  const cards = Array.from(grid.querySelectorAll('.card'));
  const items = cards.map(c => ({
    type: c.dataset.type,                  // "graph" | "table"
    src: c.dataset.src || '',              // image (graph) or table html file
    thumb: c.dataset.thumb || '',          // table thumbnail (optional)
    key: c.dataset.key || '',
    caption: c.dataset.caption || ''
  }));

  // Build structured thumbnails: graphs use data-src, tables prefer data-thumb
  items.forEach((it, i) => {
    const card = cards[i];
  ...
  // Click → open lightbox with media + description
  ...
  function openLightbox(startIndex) {
    ...
    function render(i) {
      const it = items[i];
      const copy = DESCRIPTIONS[it.key] || '';

      content.innerHTML = `
        <div class="lb-caption">${it.caption || ''}</div>
        <div class="lb-content">
          <div class="lb-media"></div>
          <div class="lb-descbox">${copy}</div>
        </div>
      `;
